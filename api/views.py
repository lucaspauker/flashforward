...
from .serializers import ContactResponseSerializer, EmailListResponseSerializer
...
class EmailListResponseView(APIView):
    def get(self, request):
        responses = EmailListResponse.objects.all()
        return Response({"responses": responses})

    def post(self, request):
        serializer = EmailListResponseSerializer(data=request.data)
        if serializer.is_valid():
            serializer.save()
            return Response({"status": "success", "data": serializer.data}, status=status.HTTP_200_OK)
        else:
